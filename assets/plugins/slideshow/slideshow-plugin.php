<?php
/**
 * Plugin Name: I-GUIDE Homepage Slideshow
...
function sp_display_active_slideshows()
{
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            ...
            if ($url) {
                echo '<a href="' . esc_url($url) . '"  class="slideshow-slide" style="background-image: url(' . esc_url($image) . ');">';
            } else {
                echo '<div  class="slideshow-slide" style="background-image: url(' . esc_url($image) . ');">';
            }

            // Caption block (inside link or div)
            echo '<div class="carousel-hover-caption">';
...
function sp_enqueue_slideshow_styles()
{
    if ((is_front_page() || is_home())) {
        wp_enqueue_style('slideshow-css', plugin_dir_url(__FILE__) . 'slideshow.css', array(), '1.0', 'all');
    }
}
add_action('wp_enqueue_scripts', 'sp_enqueue_slideshow_styles', 9999);
